    ...
    def plot(
        ...
        Arguments:
            ...
            suggest_lr (bool, optional): suggest a learning rate by
                - 'minimum loss': the point with minimum loss
        ...
        if suggest_lr:
            # 'minimum loss': the point with minimum loss
            print("Initial LR suggestion: Minimum Loss")
            min_loss_idx = None
            try:
                min_loss_idx = np.array(losses).argmin()
            except ValueError:
                print(
                    "Failed to compute the min loss, there might not be enough points."
                )
            if min_loss_idx is not None:
                print("Best Initial LR: {:.2E}".format(lrs[min_loss_idx]))
                ax.scatter(
                    lrs[min_loss_idx],
                    losses[min_loss_idx],
                    s=75,
